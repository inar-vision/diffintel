...
import { createRunner } from "../../src/analyzers";
...
describe("Analyzer runner", () => {
  ...
  it("reports unanalyzable features for unknown types", () => {
    ...
    const intent = {
      features: [
        { id: "f1", type: "unknown-type", status: "approved" as const },
      ],
    };
    const result = runner.checkFeatures(intent as any, []);
  ...
  it("skips draft features", () => {
    ...
    const intent = {
      features: [
        { id: "f1", type: "http-route", status: "draft" as const, method: "GET", path: "/test" },
      ],
    };
    const result = runner.checkFeatures(intent as any, []);
  ...
  it("classifies present, missing, and extra correctly", () => {
    ...
    const intent = {
      features: [
        { id: "list-users", type: "http-route", status: "approved" as const, method: "GET", path: "/users" },
        { id: "create-user", type: "http-route", status: "approved" as const, method: "POST", path: "/users" },
      ],
    };

    const result = runner.checkFeatures(intent as any, impls);
