...
import { CheckResult, IntentDocument } from "../../src/types";
...
describe("buildReport", () => {
  it("produces v0.2 format with unified features array", () => {
    const intent: IntentDocument = { version: "0.2", features: [] };
...
describe("diffReports", () => {
  it("detects newly implemented features", () => {
    const previous = {
      features: [{ id: "f1", result: "missing" as const, method: "GET", path: "/a" }],
      extraFeatures: [],
      summary: { complianceScore: 0 },
    };
    const current = {
      features: [{ id: "f1", result: "present" as const, method: "GET", path: "/a" }],
      extraFeatures: [],
      summary: { complianceScore: 100 },
    };
    const diff = diffReports(current as any, previous as any);
  ...
  it("detects newly missing features", () => {
    const previous = {
      features: [{ id: "f1", result: "present" as const }],
      extraFeatures: [],
      summary: { complianceScore: 100 },
    };
    const current = {
  ...
  it("detects new features added to intent", () => {
    ...
    const current = {
      features: [{ id: "f1", result: "missing" as const }],
      extraFeatures: [],
      summary: { complianceScore: 0 },
    };
    const diff = diffReports(current as any, previous as any);
